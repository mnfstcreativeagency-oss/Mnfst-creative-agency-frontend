...
const InquirySection = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setStatus({
        type: "error",
        message: "Failed to send message. Is the backend running?",
      });
    } finally {
      setLoading(false);
    }
        ...
        <div className="space-y-8">
          ...
          >
            ...
            <h2 className="text-5xl md:text-7xl font-bold font-space leading-none mb-6">
              Ready to Ignite <br />
              <span style={{ color: "var(--text-primary)" }}>Your Brand?</span>
            </h2>

            <p className="text-gray-400 text-lg md:text-xl max-w-md leading-relaxed">
              Have a vision? We translate complex ideas into digital
              masterclasses. Share your requirement and let's shape the future.
            </p>
          </motion.div>
          ...
          >
            {["Campaigns", "Branding", "Digital Products", "Motion"].map(
              (tag, i) => (
                <span
                  key={i}
                  className="px-4 py-2 rounded-full border border-[var(--border-color)] text-[var(--text-muted)] text-sm hover:bg-[var(--bg-secondary)] hover:border-[var(--text-muted)] transition-all cursor-default"
                >
                  {tag}
                </span>
              )
        ...
        <div className="w-full relative">
          ...
          >
            ...
            <div className="relative z-10">
              <h3
                className="text-2xl font-medium mb-8 flex items-center"
                style={{ color: "var(--text-primary)" }}
              >
                Start a Project
                <ArrowUpRight
                  className="ml-2 w-5 h-5 opacity-0 -translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300"
